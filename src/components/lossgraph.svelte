...
<svg width="{svgWidth}" height="{svgHeight}">
    <polyline
        ...
        stroke="blue"
        stroke-width="2"
        points={costHistory.map((cost, index) => `${scaleX(index)},${scaleY(cost)}`).join(' ')}
    />

    <!-- X-axis and labels -->
    <line x1="{padding}" y1="{svgHeight - padding}" x2="{svgWidth - padding}" y2="{svgHeight - padding}" stroke="black"/>
    {#each xTicks as tick}
        <text x={scaleX(tick.value)} y="{svgHeight - 15}" text-anchor="middle">{tick.label}</text>
        <line x1={scaleX(tick.value)} y1="{svgHeight - padding}" x2={scaleX(tick.value)} y2="{svgHeight - padding + 5}" stroke="black"/>
    {/each}

    <!-- Y-axis and labels -->
    <line x1="{padding}" y1="{padding}" x2="{padding}" y2="{svgHeight - padding}" stroke="black"/>
    {#each yTicks as tick}
        <text x="35" y={scaleY(tick.value)} dy="0.32em" text-anchor="end">{tick.label}</text>
        <line x1="{padding - 5}" y1={scaleY(tick.value)} x2="{padding}" y2={scaleY(tick.value)} stroke="black"/>
    {/each}

    <!-- Axis Titles -->
    <text x="{padding / 2}" y="{padding / 2}" text-anchor="start" font-weight="bold">Loss</text>
    <text x="{svgWidth / 2}" y="{svgHeight - 5}" text-anchor="middle" font-weight="bold">Iterations</text>
</svg>
