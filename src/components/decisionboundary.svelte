<script>
  ...
  function scaleY(y) {
    ...
    return (
      svgHeight -
      (((y - yMin) / (yMax - yMin)) * (svgHeight - 2 * padding) + padding)
    );
  }
...
<div class="container">
  <svg width={svgWidth} height={svgHeight}>
    ...
    {#each data as point}
      <circle
        cx={scaleX(point.sepalLength)}
        cy={scaleY(point.sepalWidth)}
        r="5"
        fill={point.species === 0 ? "red" : "blue"}
      />
    {/each}
    <!-- Decision boundary -->
    {#if boundaryY(4, currentWeights) !== null && boundaryY(7.5, currentWeights) !== null}
      <line
        x1={scaleX(4)}
        y1={scaleY(boundaryY(4, currentWeights))}
        x2={scaleX(7.5)}
        y2={scaleY(boundaryY(7.5, currentWeights))}
        stroke="green"
        stroke-width="2"
      />
    {/if}
