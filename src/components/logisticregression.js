function preprocessData(data) {
  return data
    .filter(
      (d) => d.Species === "Iris-setosa" || d.Species === "Iris-versicolor"
    )
    .map((d) => ({
      bias: 1,
...
function gradientDescent(X, y, lr, maxIterations, tolerance = 0.0001) {
  ...
  while (!converged && iteration < maxIterations) {
    let predictions = X.map((inputs) =>
      sigmoid(inputs.reduce((acc, input, idx) => acc + input * weights[idx], 0))
    );
    ...
    weights = weights.map(
      (w, j) =>
        ...
        lr *
          (1 / m) *
          predictions.reduce(
            (acc, pred, idx) => acc + (pred - y[idx]) * X[idx][j],
            0
          )
    );
